...
SQL2="SELECT projectName FROM Projects WHERE projectID="& Request("projectID") 
...
DO WHILE NOT RS0.EOF
	dirName = Year(RS0(1)) &"_"& Month(RS0(1)) &"_"& Day(RS0(1))  &"\"
...
'-- we must pass the inspecID and the fullPath file information for the FSO.createFile ------------------
	...
	FOR n=31 to 44 step 1
		IF NOT(TRIM(RS1(n)))="NONE" THEN 
			fileType= MID(TRIM(RS1(n)),InStrRev(TRIM(RS1(n)),"."))
			AddArcElement fileType, RS1(n), imagePath & RS1(n).Name &"\"& RS1(n), localDest & dirName & RS1(n).Name & fileType, clientDest & dirName & RS1(n).Name
'--			Response.Write( arr1(3,UBound(arr1,2)) &":"& arr1(4,UBound(arr1,2)) &"<BR>")
...
'-- file1.WriteLine("OPEN=HTMLstarter_free.exe")
'-- file1.WriteLine("ICON=HTMLstarter_free.exe")
'-- file1.close 
AddArcElement "html","Reports.html","", localDest & "Reports.html", clientDest & "Reports.html"
createReportsHTML localDest, projectName
...
<!-- #include file="../adminHeader2_dev.inc" -->
