	...
	<form id="theForm" method="post" action="<% = Request.ServerVariables("script_name") %>" onsubmit="return isReady(this)";>
		...
		<tr><td width="35%" bgcolor="#eeeeee"><img src="../../images/dot.gif" width="5" height="5"></td>
			...
			<td bgcolor="#999999"> <input type="text" name="inspecDate" size="10" maxlength="10"
					value="<% = Trim(rsReport("inspecDate")) %>"> <small>&nbsp;(mm / dd / yyyy)</small></td>
		</tr>
		<!-- project name -->
		<tr><td align="right" bgcolor="#eeeeee"><b>Project Name | Phase:</b></td>
			<td bgcolor="#999999"><input type="text" name="projectName" size="50" maxlength="50"
				value="<% = Trim(rsReport("projectName")) %>"/><input type="text" name="projectPhase" size="20" maxlength="20"
				value="<% = Trim(rsReport("projectPhase")) %>"/></td>
		</tr>
		<!-- project location -->
		<tr><td align="right" bgcolor="#eeeeee"><b>Project Location:</b></td>
			<td bgcolor="#999999"><input type="text" name="projectAddr" size="50" maxlength="50"
				value="<% = Trim(rsReport("projectAddr")) %>"> </td>
			...
			<td bgcolor="#999999"><input type="text" name="projectCity" size="20" maxlength="20"
				value="<% = Trim(rsReport("projectCity")) %>"> &nbsp; <select name="projectState">
<% 	SQL0="SELECT * FROM States ORDER BY priority DESC, stateName ASC"
	...
	DO WHILE NOT RS0.EOF %>	<option value="<%= RS0("stateAbbr")%>"<% If Trim(rsReport("projectState")) = RS0("stateAbbr") Then %> selected<% End If %>><%= Trim(RS0("stateAbbr"))%></option>
<%	RS0.MoveNext
	LOOP %>	</select> &nbsp; <input type="text" name="projectZip" size="5" maxlength="5" value="<% = Trim(rsReport("projectZip")) %>"> </td>
		</tr>
		<!-- onsite contact -->
...
<% 	SQL1="SELECT * FROM Counties ORDER BY priority DESC, countyName ASC"
	...
	DO WHILE NOT RS1.EOF %><option value="<%= Trim(RS1("countyName"))%>"<% If Trim(rsReport("projectCounty")) = TRIM(RS1("countyName")) Then %> selected<% End If %>><%= Trim(RS1("countyName"))%></option>
<%	RS1.MoveNext
	LOOP %>	</select></td>
			...
			<td bgcolor="#999999"><input type="text" name="onsiteContact" size="50" maxlength="50"
				value="<% = Trim(rsReport("onsiteContact")) %>"></td>
		</tr>
		<!-- office # -->
		<tr><td align="right" bgcolor="#eeeeee"><b>On-Site Contact:</b></td>
			<td bgcolor="#999999"><input name="officePhone" type="text" size="50" maxlength="50"
				 value="<% = Trim(rsReport("officePhone")) %>"></td>
		</tr>
		<!-- emergency # -->
		<tr><td align="right" bgcolor="#eeeeee"> <b>On-Site Contact:</b></td>
			<td bgcolor="#999999"><input name="emergencyPhone" type="text" size="50" maxlength="50"
				 value="<% = Trim(rsReport("emergencyPhone")) %>"></td>
		...
		<tr><td colspan="2"><img src="../../images/dot.gif" width="5" height="5"></td>
			...
			<td bgcolor="#999999"><input type="text" name="compName" size="50" maxlength="50"
				 value="<% = Trim(rsReport("compName")) %>"></td>
		</tr>
		<!-- Address -->
		<tr><td align="right" bgcolor="#eeeeee"><b>Address 1:</b></td>
			<td bgcolor="#999999"><input name="compAddr" type="text" size="50" maxlength="50"
				value="<% = Trim(rsReport("compAddr")) %>"></td>
...
<% 	SQL0="SELECT * FROM States ORDER BY priority DESC, stateName ASC"
	...
	DO WHILE NOT RS0.EOF %><option value="<%= Trim(RS0("stateAbbr"))%>"<% If Trim(rsReport("compState")) = RS0("stateAbbr") Then %> selected<% End If %>><%= Trim(RS0("stateAbbr"))%></option>
<%	RS0.MoveNext
	LOOP %>	</select></td>
		</tr><tr><td align="right" bgcolor="#eeeeee"><b>Zip:</b></td>
			<td bgcolor="#999999"><input name="compZip" type="text" size="5" maxlength="5"
				value="<% = Trim(rsReport("compZip")) %>"></td>
		</tr>
		...
		<tr><td align="right" bgcolor="#eeeeee"><b>Contact Phone:</b></td>
			<td bgcolor="#999999"><input name="contactPhone" type="text" size="20" maxlength="20"
				value="<% = Trim(rsReport("contactPhone")) %>"></td>
		</tr>
		<!-- fax -->
		<tr><td align="right" bgcolor="#eeeeee"><b>Contact Fax:</b></td>
			<td bgcolor="#999999"><input name="contactFax" type="text" size="20" maxlength="20"
				value="<% = Trim(rsReport("contactFax")) %>"></td>
		</tr>
		<!-- e-mail -->
		<tr><td align="right" bgcolor="#eeeeee"><b>Contact E-Mail:</b></td>
			<td bgcolor="#999999"><input type="text" name="contactEmail" size="50" maxlength="50"
				value="<% = Trim(rsReport("contactEmail")) %>"></td>
		</tr><tr><td bgcolor="#eeeeee"><img src="../../images/dot.gif" width="5" height="5"></td>
			<td bgcolor="#999999"><img src="../../images/dot.gif" width="5" height="5"></td>
		</tr><tr><td colspan="2"><img src="../../images/dot.gif" width="5" height="5"></td>
		</tr><tr><td colspan="2"><hr align="center" width="95%" size="1"></td>
...
<% 	SQL2="SELECT * FROM ReportTypes where priority > 0 ORDER BY priority DESC, reportTypeID ASC"
	...
	DO WHILE NOT RS2.EOF %><option value="<%= Trim(RS2("reportType"))%>"<% If Trim(rsReport("reportType")) = TRIM(RS2("reportType")) Then %> selected<% End If %>><%= Trim(RS2("reportType"))%></option>
<% 	RS2.MoveNext
	LOOP %>	</select></td>
		</tr>
		<!-- Rain -->
<% IF Trim(rsReport("inches")) > "-1" THEN %>
		<tr><td align="right" bgcolor="#eeeeee"><b>Inches of Rain:</b></td>
			<td bgcolor="#999999"><input type="text" name="inches" size="6" maxlength="6"
				value="<% = Trim(rsReport("inches")) %>"></td></tr>
<% ELSE %>
...
<%		End If  '-- this weeds out those already selected fileNames
	...
	Set TempImage = Nothing %>
			...
			<td bgcolor="#999999" nowrap align="right"><input type="button" style="width:100;"
...
'Else
	...
	<tr><td width="10%" rowspan="3" align="center">
	        <input type="hidden" name="coord:coID:<%= n %>" value="<%= coID %>" />
	        <input type="text" name="coord:orderby:<%= n %>" size="4" maxlength="4" value="<% = orderby %>" />&nbsp;
	        <input type="checkbox" name="coord:del:<%= n %>" /></td>
		<td width="20%" align="right"><b>Location:</b></td>
		<td width="70%"><input name="coord:coord:<%= n %>" type="text" value="<%= coordinates %>" size="50" maxlength="50"></td></tr>
<%	IF existingBMP <> "-1" THEN %>
	<tr><td align="right"><b>Existing BMP:</b></td><td><font face="Times" size="2.5pt"><%= existingBMP %></font></td></tr>
<% 	END IF %>
	<tr><td align="right" valign="top" nowrap><b>Corrective Mods:</b><!--<br />(required)--></td><td>
	    <textarea name="coord:mods:<%= n %>" cols="100" rows="10"><%= correctiveMods %></textarea></td></tr>
	<tr><td colspan="3"><hr align="center" width="100%" size="1"></td></tr>
<%	 	rsCoord.MoveNext
        n = n + 1
	Loop
...
%>	<tr><td width="10%" rowspan="3" align="center">
	        ...
	        <input type="text" name="coord:orderby:0" size="4" maxlength="4" value=""/>&nbsp;
	        <input type="checkbox" name="coord:del:0" disabled="disabled" /></td>
		<td width="20%" align="right"><b>Location:</b></td>
		<td width="70%"><input name="coord:coord:0" type="text" value="" size="50" maxlength="50" /></td></tr>
	<tr><td align="right" valign="top" nowrap><b>Corrective Mods:</b><!--<br />(required)--></td><td>
	    <textarea name="coord:mods:0" cols="100" rows="10" ></textarea></td></tr>
	<tr><td colspan="3"><hr align="center" width="100%" size="1"></td></tr>
...
basePath="http:\\www.swppp.com"
URL= basePath & "\admin\maintain\editNarrative.asp?inspecID=" & inspID
params="height=420,width=520,status=yes,toolbar=no,menubar=no, directories=no, location=no, scrollbars=no, resizable=no"
