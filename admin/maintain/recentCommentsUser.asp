<%@ Language="VBScript" %>
<%
If Not Session("validDirector") and Not Session("validInspector") Then
...
commSQLSELECT = "SELECT coID, comment, C.userID, date, I.inspecID, I.projectID FROM CoordinatesComments as C INNER JOIN Inspections as I ON C.inspecID = I.inspecID" &_
...
<% Else %>
    ...
    <% DO WHILE NOT rsComm.EOF 
        ...
        projectID = rsComm("projectID")

        If InStr(comment,"This item was marked") <> 1 Then 'returns the position that the string starts 
            
            'determine if current user can view this projectID
            projUserSQLSELECT = "SELECT * FROM ProjectsUsers" &_
                " WHERE userID=" & viewerID & " AND projectID="& projectID
            'Response.Write(projUserSQLSELECT &"</br>")
            Set rsProj = connSWPPP.execute(projUserSQLSELECT)

            If Not rsProj.EOF Then
                'Response.Write(rsProj("rights")&"</br>")
                'Get user name
