...
strBody=strBody &"<tr><td align='right'><b>Office Number:</b></td><td colspan='3'>"&  Trim(rsInspec("officePhone")) &"</td></tr>"
strBody=strBody &"<tr><td align='right'><b>Emergency Number:</b></td><td colspan='3'>"&  Trim(rsInspec("emergencyPhone")) &"</td></tr>"
...
strBody=strBody &"<p><center><i>Utilizing the Site Map, SWPPP INSPECTIONS, INC. makes the following observations:</i></center>"
...
strBody=strBody &"<tr valign='top'><td width='20%' align='right'><b>Coordinates:</b></td>	<td width='80%' align='left'>"&  coordinates &"<br></td></tr>"
...
END IF 
strBody=strBody &"<tr valign='top'><td width='20%' align='right'><b>Corrective Modifications:</b></td><td width='80%' align='left'>"&  correctiveMods &"</td></tr>"
...
IF LEN(narrative)>0 THEN strBody=strBody &"<p><small>"& REPLACE(TRIM(narrative),"#@#","'") &"</p>" END IF 
strBody=strBody &"<p><small>"& REPLACE(TRIM(qualifications),"#@#","'")&"</p>"
...
SQL3="SELECT oImageFileName FROM OptionalImages, OptionalImagesTypes " &_
     "WHERE  OptionalImages.oitID = OptionalImagesTypes.oitID "&_
	 "AND OptionalImagesTypes.oitName ='sitemap' AND inspecID="& inspecID
'response.write(SQL3)
...
If Not rsImages.EOF Then 
	strImages="<div class=indent30><b>Site Images:</b><br><br><table cellspacing=0 cellpadding=4 width='90%' border=0><tr>"
...
strBody=strBody &"<br><div align='center'><a href='http://www.swpppinspections.com'>link to: www.swpppinspections.com</a></div></Body>"
END IF
RS3.Close
SET RS3=nothing
...
projectID=projID(0)
		...
		SQL1="SELECT DISTINCT (LTRIM(RTRIM(u.firstName)) +' '+ LTRIM(RTRIM(u.lastName))) as fullName,"&_
			" u.email, u.noImages, i.projectName, i.projectPhase, i.inspecDate" &_
			" FROM ProjectsUsers pu JOIN Users u on pu.userID=u.userID" &_
			" JOIN Inspections i ON pu.projectID=i.projectID" &_
			" WHERE pu.rights='email' AND i.inspecID="& Request(Item) &" AND pu.projectID="& projectID
...
'--------------------- process mailing -------------------------------------------		
			...
			If RS1("noImages")=1 Then
	 			Mailer.BodyText = strBody=strBody &"</Body>" 
			Else
				Mailer.BodyText = strBody & strImages & "<Body>"
			End If
			Mailer.ContentType = "text/html"
'--Response.Write(Trim(RS1("fullName"))&":"& Trim(RS1("email")))
			if not Mailer.SendMail then %>
				<FONT color="red">Mail send failure.- </FONT><%= Mailer.Response %><br>
<%			else %>
				Email Sent - <%= RS1("fullName")%>:&nbsp;<%= TRIM(RS1("email"))%>&nbsp;&nbsp<%= Trim(RS1("projectName"))%>&nbsp;<%= Trim(RS1("projectPhase"))%><BR>
<%			end if
			RS1.MoveNext
		LOOP
...
'	Response.End
ELSE
SQL0 = "SELECT i.projectName, i.projectPhase, i.inspecDate, i.inspecID, pu.projectID, i.released" &_
	" FROM ProjectsUsers pu JOIN Inspections i ON pu.projectID=i.projectID" &_
	" WHERE pu.rights='inspector' AND i.released=0 AND i.userID=pu.userID AND pu.userID="& Session("userID") &_
...
<table border="0" cellpadding=1 cellspacing=1>
	<tr><th>Project Name|Phase</th><th>Report Date</th><th>send email</th></tr>
