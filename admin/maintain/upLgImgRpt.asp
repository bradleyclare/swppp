...
<html>
<head>
<title>SWPPP Inspections : Report : Large Image Upload</title>
...
<form action="<% = Request.ServerVariables("script_name") %>" enctype="multipart/form-data" method="post" onSubmit="return isReady(this)";>
		...
		<tr><td colspan="2" align="center">Upon completion, this page will redirect<br>
				to the report administration for immediate editing</td>
		</tr>
...
<% 	Else %>
		<!-- #include file="../connSWPPP.asp" --><%
			...
			imageID = upLoad.Form(2)			
			Set upLoad = Nothing			
			lgImgSQLUPDATE = "UPDATE Images SET" & _
				" largeImage = '" & lgImgFile & "'" & _
				" WHERE imageID = " & imageID
			connSWPPP.execute(lgImgSQLUPDATE)			
			connSWPPP.close
			Set connSWPPP = Nothing			
			'either add or edit image includes query sting
