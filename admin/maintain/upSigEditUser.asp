...
<html>
<head>
<title>SWPPP INSPECTIONS : Signature Upload</title>
...
<form action="<% = Request.ServerVariables("script_name") %>" enctype="multipart/form-data" method="post" onSubmit="return isReady(this)";>
		...
		<tr> 
			<td colspan="2" align="center">Upon completion, this page will redirect 
				to the user administration<br>
				for file association and immediate editing.</td>
		</tr>
		<tr> 
			<td colspan="2">&nbsp;</td>
		</tr>
		<tr> 
			<td width="25%" align="right" nowrap>Local File Name:</td>
			<td width="75%"><input name="localFile" type="file" size="50"></td>
		</tr>
		<tr> 
			<td height="44" align="left">&nbsp;</td>
			<td height="44" valign="bottom"> <input type="submit" value="Start Upload"> 
			</td>
		</tr>
	</form>
</table>
...
'	tempDev = "dev\"
	...
	upLoad.Path = baseDir & "signatures\"
...
%>
	<tr>
		<td>The file that you tried to upload was empty. Most likely, you did 
			...
			again to process</a> another image.</td>
	</tr>
	<%
	ElseIf upLoad.ContentDisposition <> "form-data" Then
	%>
	<tr>
		<td>Your upload did not succeed, most likely because your browser does 
			...
			functions</a> with this user session.</td>
	</tr>
<%
	Else
...
%>
	<tr>
		<td><font color="red">An error occurred when saving the file on the server. 
			Possible causes include:</font>
			<ul>
				<li>An incorrect filename was specified when trying to upload.</li>
				<li>File permissions do not allow writing to the specified area.</li>
			</ul>
...
<%
		Else
			...
			' sigFile = Replace(upLoad.ServerName, upLoad.Path, "")
			
			userID = upLoad.Form(1)
			
			Set upLoad = Nothing
			
			' sigSQLINSERT = "UPDATE Users SET" & _
			' 	" signature = '" & sigFile & "'" & _
			' 	" WHERE userID = " & userID
			
			' Response.Write(sigSQLINSERT & "<br><br>")
			' connSWPPP.execute(sigSQLINSERT)
			
			' connSWPPP.close
			' Set connSWPPP = Nothing
			
			Response.Redirect("editUser.asp?id=" & userID)
