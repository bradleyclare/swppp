...
<body>
	...
	<h3>Actions Taken Report<br>for <%= RS2("projectName") %> <%= RS2("projectPhase")%></h3>
	<table width="100%">
	<tr><th width="30%">Date</th><th width="70%">Action Taken</th></tr>
<% 	DO WHILE NOT RS1.EOF 
		canEdit=false
		IF Session("validAdmin") OR Session("validDirector") OR Session("userID")=RS1(4) THEN 
			canEdit=true 
		END IF %>
		<tr <% IF canEdit THEN %>
			onMouseOver="this.className='highlighted';" onMouseOut="this.className='';" onClick="window.location='editActionReport.asp?ID=<%=RS1(0) %>&pID=<%= projectID %>';"
		<% END IF %>><td align=left><%= RS1(1)%><BR><%=Trim(RS1(2))%></td>
		<td align=left><%=Trim(UnCleanText(RS1(3)))%>
		</td>
		</tr> 
<%		RS1.MoveNext
 	LOOP 
